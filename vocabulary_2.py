...
class vocabulary(object):
    ...
    def look_up_one(self,word):
        ...
        try:
            ...
            for i, name in enumerate(pos_list):
                ...
                acceptation=OpenCC('s2tw').convert(acceptation)
    ...
    def process_word(word):
        # 处理单词的逻辑，这里使用了 sleep 模拟耗时操作
        import time
        time.sleep(1)  # 模拟耗时操作
        return word.upper()  # 返回大写单词作为示例
    ...
    def write_words(self,data):
        with open(self.save_filename,'w',encoding='utf-8') as f:
            for datum in data:
                ...
                text = '{}\t{}\n'.format(datum['key'],datum['acceptation'])
                f.write(text)
    ...
    def download_audio(self,data):
        ...
        with tqdm(total = len_words,desc='Downloading sudio') as fbar:
            ...
            for i, pos in enumerate(len_words):
                j = i+1
                key = data[i]['key']
                url = data[i]['pron']
                semaphore.acquire()
